...
def _is_excluded(rel_path: str, exclude_patterns: list[str]) -> bool:
    ...
    Supports ``**`` glob segments: a pattern like ``**/migrations/**`` will
    match any path that contains a ``migrations`` directory component.
    """
    for pattern in exclude_patterns:
        # fnmatch doesn't handle ** well, so we normalise:
        # "**/X/**" -> any path containing "/X/" or starting with "X/"
        # We check both the full relative path and with a leading "/" to
        # handle top-level directories.
        if fnmatch.fnmatch(rel_path, pattern):
            return True
        if fnmatch.fnmatch("/" + rel_path, "/" + pattern.lstrip("*")):
            return True
        # Simple component check: if pattern is **/dirname/**, check if
        # dirname appears as a path component.
