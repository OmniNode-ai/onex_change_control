...
class TestDualControlRemovedOMN14814:
    """The SCHEMA-ONLY invocation (no requester context) is unchanged.

    Historical note, kept because the behaviour it pins is still load-bearing:
    OMN-14814 removed the `approved_by != PR-author` check outright, on the
    premise of a sole CODEOWNER. OMN-17157 restored it — but REQUESTER-SCOPED,
    so `validate_grants(path)` with no requester is still pure schema
    validation and every assertion below still holds verbatim. That is the
    contract this class now pins: the self-approval check must never fire
    without an explicit requester identity, or a local `--file` run and every
    non-pull_request event would start refusing valid files.

    See TestSelfApprovalRefusedOMN17157 for the restored control itself.
    """
...
# ---------------------------------------------------------------------------
# OMN-17157: restore a mechanical approver != requester control on the grant
# anchor.
#
# OMN-14814 removed the former `approved_by != PR-author` check on the stated
# premise that `@OmniNode-ai/platform-leads` "has exactly one member (the sole
# CODEOWNER)", making a second, different approver structurally unsatisfiable.
# That premise is no longer true: the team has TWO members, so the control is
# satisfiable again and its absence is a real hole rather than a concession.
#
# The check is REQUESTER-SCOPED and DIFF-SCOPED, exactly as OMN-14441's was:
#   * with no requester context supplied it does not run at all (a local
#     `--file`-only invocation validates schema and nothing more), which is
#     why TestDualControlRemovedOMN14814 above still passes verbatim; and
#   * with a base file supplied it fires only on entries this change ADDS,
#     so an unrelated PR opened by someone who happens to share a login with
#     the approver of an untouched entry is never false-positived.
#
# The reason string is `self_granted` — the same token omninode_infra's
# promotion-time gate emits (EnumGrantOutcome.SELF_GRANTED) — so one grep
# finds both halves of the control.
# ---------------------------------------------------------------------------


def _base_file_with(path: Path, entries: list[dict[str, Any]]) -> Path:
    """Render a 'grants file as it exists on the base ref' for diff-scoping."""
    _write_grants(path, entries)
    return path


class TestSelfApprovalRefusedOMN17157:
    """A grant whose approved_by IS the requesting identity is refused."""

    def test_self_approved_new_entry_is_refused(self, tmp_path: Path) -> None:
        grants_file = tmp_path / "grants.yaml"
        base_file = _base_file_with(tmp_path / "base.yaml", [])
        _write_grants(grants_file, [_future_entry(approved_by="jonahgabriel")])

        result = validate_grants(
            grants_file, requester="jonahgabriel", base_file=base_file
        )

        assert not result.passed, (
            "a grant approved by the same identity that requested it must be "
            "refused at review time, not left to the promotion-time gate"
        )
        assert any("self_granted" in e for e in result.errors), result.errors

    def test_self_approval_match_is_case_insensitive(self, tmp_path: Path) -> None:
        """GitHub logins are case-insensitive; casing must not launder a grant."""
        grants_file = tmp_path / "grants.yaml"
        base_file = _base_file_with(tmp_path / "base.yaml", [])
        _write_grants(grants_file, [_future_entry(approved_by="JonahGabriel")])
    ...
    def test_cross_approved_new_entry_passes(self, tmp_path: Path) -> None:
        """POSITIVE CONTROL: the check must not refuse a real two-person grant.

        `@OmniNode-ai/platform-leads` has two members, so this is a reachable
        state — which is the whole reason OMN-14814's premise no longer holds.
        """
        grants_file = tmp_path / "grants.yaml"
        base_file = _base_file_with(tmp_path / "base.yaml", [])
        _write_grants(grants_file, [_future_entry(approved_by="jake-b-omni")])
    ...
    def test_preexisting_self_approved_entry_is_not_refused(
        self, tmp_path: Path
    ) -> None:
        """DIFF-SCOPING: an entry this change did not add is out of scope.

        Without this, any unrelated PR opened by the approver of a live grant
        would redden — the false-positive OMN-14441 diff-scoped against.
        """
        entry = _future_entry(approved_by="jonahgabriel")
        grants_file = tmp_path / "grants.yaml"
        base_file = _base_file_with(tmp_path / "base.yaml", [entry])
        _write_grants(grants_file, [entry])
    ...
    def test_absent_base_file_treats_every_entry_as_new(self, tmp_path: Path) -> None:
        """FAIL-CLOSED: if newness cannot be established, every entry is new.

        An unreadable or missing base is not evidence that an entry is
        pre-existing, so it must not buy an entry a pass.
        """
        grants_file = tmp_path / "grants.yaml"
        _write_grants(grants_file, [_future_entry(approved_by="jonahgabriel")])

        result = validate_grants(
            grants_file, requester="jonahgabriel", base_file=tmp_path / "nope.yaml"
        )

        assert not result.passed
        assert any("self_granted" in e for e in result.errors), result.errors

    def test_no_requester_context_skips_the_check(self, tmp_path: Path) -> None:
        """POSITIVE CONTROL for the skip path: schema-only invocation is unchanged.

        This is the invocation `uv run validate-prod-promotion-grants --file ...`
        makes locally, and the one TestDualControlRemovedOMN14814 exercises.
        """
        grants_file = tmp_path / "grants.yaml"
        _write_grants(grants_file, [_future_entry(approved_by="jonahgabriel")])
    ...
    def test_other_schema_failures_still_fire_alongside_self_approval(
        self, tmp_path: Path
    ) -> None:
        """The new check must not short-circuit the checks that already held."""
        entry = _future_entry(approved_by="jonahgabriel", image_digest="not-a-digest")
        grants_file = tmp_path / "grants.yaml"
        base_file = _base_file_with(tmp_path / "base.yaml", [])
        _write_grants(grants_file, [entry])
        ...
        assert not result.passed
        assert any("image_digest must match" in e for e in result.errors), result.errors


class TestSelfApprovalWiringOMN17157:
    """Both halves of the control are WIRED, not merely implemented.

    Rule 5: a check that is not wired as a gate is advisory and gets ignored.
    The two halves cover two different authoring paths and neither is
    sufficient alone, so both wirings are asserted here.
    """

    _CI = _REPO_ROOT / ".github" / "workflows" / "ci.yml"
    _STAGE = _REPO_ROOT / ".github" / "workflows" / "stage-prod-promotion-grant.yml"

    def _ci_step(self) -> str:
        text = self._CI.read_text(encoding="utf-8")
        marker = "Validate prod-promotion-grants trust anchor"
        assert marker in text, f"{self._CI} no longer runs the grants validator"
        start = text.index(marker)
        return text[start : start + 3000]

    def test_ci_passes_a_requester_to_the_validator(self) -> None:
        """Without --requester the check is inert, so CI must supply it.

        Asserts the EXECUTABLE construct, not the mere presence of the string:
        the step's own comments mention `--requester`, so a substring test
        would stay green with the argument deleted. (It did, until a positive
        control that removed the line caught the false pass.)
        """
        step = self._ci_step()
        assert 'args+=(--requester "${pr_author}")' in step, (
            "ci.yml must APPEND --requester to the validator argv; without it "
            "the OMN-17157 self-approval check never runs and the job is "
            "schema-only again"
        )

    def test_ci_resolves_the_requester_from_the_github_api(self) -> None:
        """Not from the YAML, and not from the (snapshot) event payload."""
        step = self._ci_step()
        reason = (
            "the requester identity must be resolved from the authenticated "
            "GitHub API — a requester can hand-type any name into approved_by"
        )
        assert 'pr_author="$(gh api' in step, reason
        assert ".user.login" in step, reason

    def test_ci_fails_closed_when_the_pr_author_is_unresolvable(self) -> None:
        step = self._ci_step()
        assert "exit 1" in step, (
            "a pull_request with no resolvable author must FAIL the step, "
            "not fall through to an unscoped (and therefore inert) run"
        )

    def test_ci_diff_scopes_against_the_base_revision(self) -> None:
        step = self._ci_step()
        assert 'args+=(--base-file "${base_grants}")' in step, (
            "without --base-file every unrelated PR by a live grant's approver "
            "would redden — the false positive OMN-14441 diff-scoped against"
        )

    def test_staging_workflow_refuses_a_self_dispatched_grant(self) -> None:
        """The canonical authoring path opens its PR as a GitHub App.

        So the PR author is never the human there and the validator's
        comparison is vacuous on that path; the dispatch-time check is the
        only place it can bite.
        """
        text = self._STAGE.read_text(encoding="utf-8")
        assert "DISPATCH_ACTOR" in text, (
            "the staging workflow must expose the dispatching actor — it is "
            "the requesting human on the App-authored path"
        )
        assert "self_granted" in text, (
            "the staging workflow must refuse approved_by == dispatcher with "
            "the same reason string the promotion-time gate uses"
        )

    def test_both_halves_use_the_same_reason_string(self) -> None:
        """One condition, one token, so a grep finds every refusal of it."""
        from onex_change_control.scripts.validate_prod_promotion_grants import (
            SELF_GRANTED_REASON,
        )

        assert SELF_GRANTED_REASON == "self_granted"
        assert SELF_GRANTED_REASON in self._STAGE.read_text(encoding="utf-8")
