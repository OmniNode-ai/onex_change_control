...
from run_contract_compliance_check import (  # type: ignore[import]
    _RESULT_BLOCK,
    _RESULT_PASS,
    _RESULT_WARN,
    _check_command,
...
def test_extract_ticket_id_from_title(tmp_path: Path) -> None:
    pr_json = '{"title": "fix: something [OMN-1234]", "headRefName": "main", "body": ""}'
...
def test_extract_ticket_id_from_branch(tmp_path: Path) -> None:
    pr_json = '{"title": "no ticket here", "headRefName": "jonah/omn-5678-my-fix", "body": ""}'
    with patch("run_contract_compliance_check._run", return_value=(0, pr_json, "")):
        result = _extract_ticket_id(42, "OmniNode-ai/omnimarket")
...
def test_extract_ticket_id_gh_failure() -> None:
    with patch("run_contract_compliance_check._run", return_value=(1, "", "auth error")):
        result = _extract_ticket_id(42, "OmniNode-ai/omnimarket")
    assert result is None
...
def test_find_contracts_dir_local_fallback(tmp_path: Path) -> None:
    # When the script lives at tmp_path/scripts/ci/ and contracts/ is at tmp_path/contracts/,
    # _find_contracts_dir should return the sibling contracts dir relative to the script's repo root.
...
def test_check_test_exists_pass(tmp_path: Path) -> None:
    ...
    result, detail = _check_test_exists("tests/test_*.py", tmp_path)
    assert result == _RESULT_PASS


def test_check_test_exists_block(tmp_path: Path) -> None:
    result, detail = _check_test_exists("tests/test_nonexistent_*.py", tmp_path)
...
def test_check_grep_block(tmp_path: Path) -> None:
    result, _ = _check_grep({"pattern": "definitely_not_here_xyz", "path": "."}, tmp_path)
    assert result == _RESULT_BLOCK


def test_check_grep_bad_value() -> None:
    result, detail = _check_grep("not-a-dict", Path("/tmp"))
    assert result == _RESULT_BLOCK
...
def test_emergency_bypass_skips_all_checks(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_no_ticket_id_returns_pass(tmp_path: Path) -> None:
    with patch("run_contract_compliance_check._extract_ticket_id", return_value=None):
        rc = run_compliance_check(1, "OmniNode-ai/omnimarket", tmp_path / "contracts", tmp_path)
    assert rc == 0


def test_no_contract_file_returns_pass_with_warn(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    contracts = tmp_path / "contracts"
    contracts.mkdir()
    with patch("run_contract_compliance_check._extract_ticket_id", return_value="OMN-9999"):
...
def test_contract_with_no_dod_evidence_returns_pass(tmp_path: Path) -> None:
    ...
    with patch("run_contract_compliance_check._extract_ticket_id", return_value="OMN-1001"):
        rc = run_compliance_check(1, "OmniNode-ai/omnimarket", contracts, tmp_path)
    assert rc == 0
...
def test_contract_with_passing_file_exists_check(tmp_path: Path) -> None:
    ...
    with patch("run_contract_compliance_check._extract_ticket_id", return_value="OMN-1002"):
        rc = run_compliance_check(1, "OmniNode-ai/omnimarket", contracts, tmp_path)
    assert rc == 0
...
def test_contract_with_failing_check_returns_block(tmp_path: Path) -> None:
    ...
    with patch("run_contract_compliance_check._extract_ticket_id", return_value="OMN-1003"):
        rc = run_compliance_check(1, "OmniNode-ai/omnimarket", contracts, tmp_path)
    assert rc == 1
