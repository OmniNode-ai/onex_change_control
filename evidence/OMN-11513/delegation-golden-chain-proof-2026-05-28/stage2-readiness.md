...
Lane: stability-test ONLY (.201 — runtime :18085, effects :18086, Redpanda :39092, Postgres :15436)
...
- Stability-test lane is healthy on .201: runtime + effects v0.37.2 (Up, healthy), Redpanda :39092 reachable, all 6 ombase-infra chain topics already exist.
- LLM endpoint :8001 (Qwen3.6-27B) healthy on .201 → the call effect can do a live inference, so the chain can reach `delegation-completed` (pass), not just `delegation-failed`.
...
Two viable manual paths, both manual shared-infra ops on .201:
...
context on .201 (rsync from worktree or git fetch into the deploy-src snapshot).

## Verdict so far

BLOCKED on execution-path decision (1 vs 2) before touching .201 build/deploy.
