...
| 1 Broker | aiokafka metadata (api 2.8.0) | dev/`27f4f03` | n/a | Tailscale | `<onex-host>:39092` | advertises `<onex-host>` (AM: `<onex-host>`) | **PROOF / cleared** |
...
**1. OMN-12832 — broker advertised LAN address (cleared).** AM: broker at `<onex-host>:39092` accepted TCP over Tailscale but advertised `<onex-host>` in metadata, unreachable from our seat; every bus round-trip failed after the handshake. PM, after Jonah recreated the broker: the sanctioned aiokafka client (api 2.8.0) reports `brokers: [(0, '<onex-host>', 39092)]`, and the Phase 3 round-trip — which had failed at `bus.start()` with `InfraTimeoutError` — now completes in 2.8s. Retest confirmation posted to the ticket.

**2. Phase 0 endpoint override defect (observed, not closed here).** The three unit failures are not a `model_registry.local.yaml` false alarm. `phase0-unit-tests.txt` shows endpoint resolution returning `http://<onex-host>:8000/v1/chat/completions` where tests expected `localhost` or the `LOCAL_INFERENCE_BASE_URL_ENV` override. The failing `test_load_default_registry_honors_env_override` specifically verifies the env override path and shows it being ignored, so this report does not claim "no code regression" for Phase 0.
