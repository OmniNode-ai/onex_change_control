...
**Day timeline:**
- **AM:** blocked at broker reachability (metadata advertised LAN address). Filed OMN-12832.
...
| 1 Broker | aiokafka metadata (api 2.8.0) | dev/`27f4f03` | n/a | Tailscale | `100.109.203.94:39092` | advertises `100.109.203.94` (AM: LAN address) | **PROOF / cleared** |
...
**1. OMN-12832 — broker advertised LAN address (cleared).** AM: broker at `100.109.203.94:39092` accepted TCP over Tailscale but advertised the LAN address in metadata, unreachable from our seat; every bus round-trip failed after the handshake. PM, after the broker was recreated: the sanctioned aiokafka client (api 2.8.0) reports `brokers: [(0, '100.109.203.94', 39092)]`, and the Phase 3 round-trip — which had failed at `bus.start()` with `InfraTimeoutError` — now completes in 2.8s. Retest confirmation posted to the ticket.
